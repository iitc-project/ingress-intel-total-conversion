...
// @description    [@@BUILDNAME@@-@@BUILDDATE@@] linkableArea
...
window.plugin.highlightLinkableArea = function () {
};

var self = window.plugin.highlightLinkableArea;
...
self.findPolyPoints = function(guid) {
    ...
    $.each(links, function(guid, link) {
        ...
        for (var i = 0; i <= 1; i++) {
            var visible = true;
            for (var linkToCheck in window.links) {
                //noinspection JSUnfilteredForInLoop
                if (self.greatCircleArcIntersect(latlng, link.latlngs[i], linkToCheck.latlngs[0], linkToCheck.latlngs[1])) {
                    visible = false;
                    break;
                }
            }
            if (visible) {
                points.push(link.latlngs[i]);
            }
        }
    });
    return points;
...
self.sortPointsByAngle = function(points, guid) {
    ...
    points.sort(function(a, b) {
        ...
        // treating the latlongs as they where normal coordinats, to make this easier. This should work fine for
        // If you're zoomed in, but shouldn't if the area is to big.
        // which shouldn't be a problem, because this plugin isn't working as good on bigger areas anyway.
        var difXa = a.lat - latlng.lat;
        var difYa = a.lng - latlng.lng;

        var difXb = b.lat - latlng.lat;
        var difYb = b.lng - latlng.lng;

        return  Math.atan(difYa/difXa) - Math.atan(difYa/difXa) ;
    });
}

self.layer = null;

self.highlightArea(guid) {
    var points = self.sortPointsByAngle(self.findPolyPoints(guid), guid);
    $.each(points, function(id, point) {
        window.highlightPortal(window.getPorta)
    })
}
...
self.setup = function () {
    ...
    window.map.on('layerremove', function (e) {
        if (e.layer === self.layer) {
            self.clearErrorMarker();
        }
    });

    window.addLayerGroup('Linkable Area Highlights', self.layer, false);
}
