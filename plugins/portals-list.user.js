...
window.plugin.portalslist.portalTable = function(sortBy, sortOrder, filter) {
    ...
    container.append('<div class="disclaimer">Click on portals table headers to sort by that column. ' +
        ...
        '<br /><a onclick="window.plugin.portalslist.maxFieldList()">Display MaxField-friendly list</a></div>');
...
window.plugin.portalslist.maxFieldList = function() {
    var maxFieldList = '';
    $('.portalTitle a').each(function() {
        maxFieldList += $(this).text() + '; https://www.ingress.com' + $(this).attr('href') + "\n";
    });
    dialog({
        html: $('<div">').append('<textarea style="width:100%;height:250px;">' + maxFieldList + '</textarea>'),
        dialogClass: 'maxFieldList',
        title: 'MaxField-Friendly List: ',
        id: 'maxFieldList',
        width: 700
    });
};
