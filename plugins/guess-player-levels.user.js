...
window.plugin.guessPlayerLevels.extractPortalData = function(data) {
  ...
  var certain;
  var minLevel;
  var minplayerlevel = function(count, level) {
  ...
  for(var nickname in players) {
    ...
    players[nickname].forEach(minplayerlevel);
...
window.plugin.guessPlayerLevels.extractChatData = function(data) {
  ...
  data.result.forEach(function(msg) {
    ...
    if(plext.plextType == 'SYSTEM_NARROWCAST' &&
      ...
      nick = plext.markup[5][1].plain;
      portal = plext.markup[3][1];
      addAttackMessage(nick, msg[1], portal);
    }
    ...
    if(plext.plextType == 'SYSTEM_NARROWCAST' &&
      ...
      nick = plext.markup[3][1].plain;
      portal = plext.markup[1][1];
      addAttackMessage(nick, msg[1], portal);
     }
  });

  var addposition = function(portal) {
  ...
  for(var nick in attackData) {
    for(var timestamp in attackData[nick]) {
      // remove duplicates
      var latlngs = [];
      var portals = {};
      attackData[nick][timestamp].forEach(addposition);
...
window.plugin.guessPlayerLevels.handleAttackData = function(nick, latlngs) {
  ...
  latlng = L.latLng(circle.y, circle.x);
  var range = 0;
